...
if __name__ == "__main__":
    ...
    num_iter   = 16         # -- Number of ADMM Iterations
    sigma      = 15
    ...
    # -- The Model
    model = video_denoise(frame_n=n_subexp, num_iter=16).to(device)
    ...
    for epoch in range(n_epoch):
        ...
        for i, data in tqdm(enumerate(train_gen)):
            ...
            loss_tv = 0.1 * TVLoss(output[-1])
            loss_tc = 0.1 * TCLoss(output[-1])
            loss = loss_tv + loss_tc
            for n, x in enumerate(output):
                x_ = torch.flatten(x, end_dim=1)
                if n == len(output) - 1:
                    loss += MSELoss(x_, target_)
                else:
                    loss += 0.2 * MSELoss(x_, target_)
    
            # print ("[INFO] loss: ", loss)
    
            # -- Clamp the output prior to computing the PSNR
            output = torch.clamp(output[-1], min=-1., max=1.)
